...
// console.log(searchParams);
  ...
  onMapRegionChange(region) {
    // this.setState({ mapRegion:region });
    if( this.state.isPreviewVisible !== false )
    ...
    if (Platform.OS === 'ios') {
      ...
      LocationServicesDialogBox.checkLocationServicesIsEnabled({
        ...
        this.watchID = navigator.geolocation.watchPosition((position) => {
          ...
          // this.mapView && this.mapView.animateToCoordinate(locationMe);
          this.mapView && this.mapView.animateToRegion(locationMe);
          // this.setState({mapRegion:locationMe});
  ...
  renderMarker(marker, index) {
    let key = marker.objectID;
    let lat = marker.locSearch[1];
    let lng = marker.locSearch[0];

    for( let i = index + 1; i < this.props.algolia.mainProperties.length; i ++ ){
      let tempLat = this.props.algolia.mainProperties[i].locSearch[1];
      let tempLng = this.props.algolia.mainProperties[i].locSearch[0];
    ...
    let IsSelectedMarker = false;
    if( this.props.algolia.selectedProperty && this.props.algolia.selectedProperty.objectID == marker.objectID )
      IsSelectedMarker = true;

    return (
      <MapView.Marker
        key= {key}
        onPress = {(e)=>{e.stopPropagation();this.onMapMarkerSelected(marker)}}
        coordinate={{
          latitude: lat,
          longitude: lng,
        }}>
      ...
      <View style={styles.container}>
          <MapView
            ...
            { this.props.algolia.mainProperties !== null &&
              this.props.algolia.mainProperties.map( this.renderMarker.bind(this) )}
          </MapView>
