...
class Search extends Component {
  ...
  renderSearchedSchoolsItem(item, index) {
    ...
    caption = item.school
    if( item.city )
      caption = caption + "-" + item.city;
    if( item.state )
      caption = caption + ", " + item.state;
      ...
      <View style={{flexDirection:'row',
        ...
        <View style={{flex:17, borderColor:"#D0D0D0",  borderBottomWidth:1, paddingBottom:5}}>
          <Text style={{ width: Metrics.screenWidth * 0.85, 
                        fontSize:15 }} numberOfLines={1}> {caption} </Text>
        </View>
      </View>
    );    
  }
  ...
  renderSearchedCommunitiesItem(item, index) {
      ...
      let caption = "";
      caption = item.community + "-" + item.city
      if( item.state )
        caption = caption + ", " + item.state;
  ...
  renderSearchedPropertiesItems(item, index) {
    if( item.address == null )
      return null;
// console.log("renderSearchedPropertiesItems");
// console.log(item);
      let caption = "";
      caption = item.address + "-" + item.city
      if( item.state )
        caption = caption + ", " + item.state;
    ...
    return (
      <View style={{width:Metrics.screenWidth }}>
        <View style={{flexDirection:'row',
          ...
          <View style={{flex:17}}>
            <Text style={{ fontWeight:'bold' }}> Nearby Me </Text>
          </View>
        </View>

        {
          this.props.algolia.mainProperties !== null && this.props.algolia.mainProperties !== undefined ? 
          this.props.algolia.mainProperties.map((item, index) => (
            this.renderSearchedNearMeItems(item, index)
          )) : 
          null
        }
  ...
  renderSearchedNearMeItems(item, index) {
    if( item.address == null )
      return null;
